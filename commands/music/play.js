...
module.exports = class PlayCommand extends Command {
  ...
  run(msg, { query }, addMessage) {
    if(query) {
      ...
      (function play(song, addMessage) {
        ...
        if(addMessage) {
          addMessage.edit(message, {embed: getEmbed(song)});
        } else {
          msg.channel.send(message, {embed: getEmbed(song)});
        }
      ...
      })(playlist.songs.shift(), addMessage);
    }
  }
  ...
  addSong(url, msg, response) {
    ...
    ytdl.getInfo(url, (err, info) => {
      ...
      var song = {url: url, title: title, artist: artist, media: info.media, duration: info.length_seconds, thumbnail: thumbnailUrl, requester: msg.author.username};
      playlist.songs.push(song);
      response.edit('Added:', {embed: getEmbed(song)});
